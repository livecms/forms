            ...
            '<div class="form-group" id="'.$name.'__parent">'.
                ...
                '<div id="preview__'.$name.'" data-image="'.$name.'" class="'.($exists ? 'exists' : ''). ' image-frame text-center" style="max-width: 280px" '.(isset($attributes['required']) ? 'required' : '').'>'.
                    ...
                    '<a class="btn btn-fill btn-block midia-toggle" data-preview="image_preview__'.$name.'" '.($value ? 'data-midia-initial_preview=""' : '').'>'.
                        '<span class="media-select" @if ($exists) style="display: none;" @endif>'. __('Select Image') .'</span>'.
                        '<span class="media-change" @if (!$exists) style="display: none;" @endif>'. __('Change Image') .'</span>'.
                    '</a>'.
                    '<a class="btn btn-fill btn-block midia-remove" for="'.$name.'" style="background-color: #bf6a6a;">'. __('Remove') .'</a>'.
